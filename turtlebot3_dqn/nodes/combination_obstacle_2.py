        ...
        while not rospy.is_shutdown():
            ...
            for i in range(len(model.name)):
                if model.name[i] == 'obstacle_2':
                    ...
                    if state == 0:
                        obstacle_2.pose.position.x += 0.007 * accelaration
                        obstacle_2.pose.position.y -= 0.001 * accelaration
                        if abs(obstacle_2.pose.position.x + 1.3) < 0.05 and abs(obstacle_2.pose.position.y + 2.1) < 0.05:
                            state = 1

                    elif state == 1:
                        obstacle_2.pose.position.x += 0.009 * accelaration
                        obstacle_2.pose.position.y += 0.009 * accelaration
                        if abs(obstacle_2.pose.position.x + 0.4) < 0.05 and abs(obstacle_2.pose.position.y + 1.2) < 0.05:
                            state = 2

                    elif state == 2:
                        obstacle_2.pose.position.x -= 0.006 * accelaration
                        obstacle_2.pose.position.y += 0.006 * accelaration
                        if abs(obstacle_2.pose.position.x + 1) < 0.05 and abs(obstacle_2.pose.position.y + 0.6) < 0.05:
                            state = 3

                    elif state == 3:
                        obstacle_2.pose.position.x += 0.008 * accelaration
                        obstacle_2.pose.position.y -= 0.001 * accelaration
                        if abs(obstacle_2.pose.position.x + 0.2) < 0.05 and abs(obstacle_2.pose.position.y + 0.7) < 0.05:
                            state = 4

                    elif state == 4:
                        obstacle_2.pose.position.x += 0.013 * accelaration
                        obstacle_2.pose.position.y -= 0.013 * accelaration
                        if abs(obstacle_2.pose.position.x - 1.1) < 0.05 and abs(obstacle_2.pose.position.y + 2) < 0.05:
                            state = 5

                    elif state == 5:
                        obstacle_2.pose.position.x += 0.009 * accelaration
                        obstacle_2.pose.position.y += 0.01 * accelaration
                        ...
                        obstacle_2.pose.position.x -= 0.007 * accelaration
                        obstacle_2.pose.position.y += 0.02 * accelaration
                        if abs(obstacle_2.pose.position.x - 0.5) < 0.05 and abs(obstacle_2.pose.position.y - 2) < 0.05:
                            state = 8

                    elif state == 8:
                        obstacle_2.pose.position.x -= 0.026 * 0.5 * accelaration
                        obstacle_2.pose.position.y -= 0.002 * 0.5 * accelaration
                        if abs(obstacle_2.pose.position.x + 2.1) < 0.05 and abs(obstacle_2.pose.position.y - 1.8) < 0.05:
                            state = 9

                    elif state == 9:
                        obstacle_2.pose.position.x += 0.016 * 0.5 * accelaration
                        obstacle_2.pose.position.y -= 0.016 * 0.5 * accelaration
                        if abs(obstacle_2.pose.position.x + 0.5) < 0.05 and abs(obstacle_2.pose.position.y - 0.2) < 0.05:
                            state = 10

                    elif state == 10:
                        obstacle_2.pose.position.x -= 0.004 * 0.5 * accelaration
                        obstacle_2.pose.position.y -= 0.023 * 0.5 * accelaration
                        if abs(obstacle_2.pose.position.x + 0.9) < 0.05 and abs(obstacle_2.pose.position.y + 2.1) < 0.05:
                            state = 11

                    elif state == 11:
                        obstacle_2.pose.position.x -= 0.011 * accelaration
                        obstacle_2.pose.position.y += 0.001 * accelaration
                        if abs(obstacle_2.pose.position.x + 2) < 0.05 and abs(obstacle_2.pose.position.y + 2) < 0.05:
                            state = 0
