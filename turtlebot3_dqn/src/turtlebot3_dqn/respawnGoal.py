    ...
    def getPosition(self, position_check=False, delete=False):
        ...
        if ENV in (1, 2, 3):
            ...
            while position_check:
                ...
                distance = sqrt((goal_pose_list[self.index][0] - goal_pose_list[self.last_index][0])**2 +
                                (goal_pose_list[self.index][1] - goal_pose_list[self.last_index][1])**2)
                # rospy.loginfo("Last goal: (%.2f, %.2f), Current goal: (%.2f, %.2f), Distance: %.2f", 
                #               goal_pose_list[self.last_index][0], goal_pose_list[self.last_index][1], 
                #               goal_pose_list[self.index][0], goal_pose_list[self.index][1], 
                #               distance)
                if self.last_index == self.index or distance < 3:
                    position_check = True
                else:
