...
import numpy as np
import random
from ros1_ws.src.turtlebot3_machine_learning.turtlebot3_dqn.utils.models import ReplayMemory, DQN, DuelDQN, RainbowDQN, DDPG, TD3, A2C, Transition
...
class DDQNAgent():
    ...
    def trainModel(self):
        ...
        with torch.no_grad():
            # double dqn
            argmax_a = self.model(next_state_batch).argmax(dim=1).unsqueeze(-1)
            next_state_values = self.target_model(next_state_batch).gather(1,argmax_a).squeeze(1)
